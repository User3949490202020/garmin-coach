# 💡 Boîte à idées — Coach Running Garmin

Idées d'évolutions pour l'application. Pour en ajouter une, dis simplement à
Claude : **« ajoute à la boîte à idées : ... »** — pas besoin d'ouvrir ce
fichier toi-même. Pour voir la liste : **« montre-moi la boîte à idées »**.

## 🆕 En attente

- **Compatibilité montres Suunto** : faire évoluer l'appli pour supporter les
  montres Suunto en plus de Garmin (aujourd'hui 100% dépendant de la
  librairie `garminconnect`, spécifique à Garmin — à voir s'il existe un
  équivalent côté Suunto, sinon ça demande une vraie refonte de la couche
  de connexion).

## 🚧 En cours

_(rien pour l'instant)_

## ✅ Faites

- **Compatibilité Suunto (et toutes marques) via Strava** — connexion Strava
  en alternative à Garmin (activités + coach IA ; pas de sommeil/récup,
  limite de Strava). Livré le 09/07/2026.
- **Onglet "🗓️ Planificateur"** (idée de Guillaume) : programme ~3 mois basé
  sur les données réelles, semaines typées *progression / stabilité /
  récupération* (+ affûtage/course si objectif), détail jour par jour des
  semaines en cours et à venir. Livré le 17/07/2026.
- **Muscu/renfo dans la charge d'entraînement** — les séances de musculation
  comptent dans l'ACWR et la fatigue. Livré le 09/07/2026.

## 📦 Mis de côté (récupérables via l'historique git)

- **Onglet Planificateur** — retiré le 23/07/2026 à la demande de Vincent
  (« pas au niveau »). Code complet dans l'historique git (commit `afba74d`)
  et `planner.py` toujours dans le dépôt. À retravailler avant de le remettre.
- **Onglet Recommandations** — retiré le 23/07/2026. Même chose : code dans
  l'historique git, à améliorer avant un éventuel retour. (Contenait aussi
  l'ajout de course objectif — à réintégrer ailleurs si besoin.)

## 🐛 Bugs connus / suivis

_(rien pour l'instant)_
